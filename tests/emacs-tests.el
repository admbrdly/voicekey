...
(defmacro voicekey-test-buffer (text &rest body)
  `(let ((voicekey--pins nil) (voicekey--operations nil) (voicekey--restore-normal nil)
...
(ert-deftest voicekey-evil-overlapping-pins-share-insert-state-until-the-last-closes ()
  (skip-unless (require 'evil nil t))
  (dolist (order '(("first" "second") ("second" "first")))
    (voicekey-test-buffer "a b"
      (evil-local-mode 1)
      (evil-normal-state)
      (goto-char 1)
      (voicekey--pin "first" (voicekey-test-expiry))
      (voicekey--pin "second" (voicekey-test-expiry))
      (should (= (point) 2))
      (voicekey--unpin (car order))
      (should (eq evil-state 'insert))
      (should (= (point) 2))
      (voicekey--unpin (cadr order))
      (should (eq evil-state 'normal))
      (should-not voicekey--restore-normal))))

(ert-deftest voicekey-evil-overlapping-pins-do-not-own-user-insert-state ()
  (skip-unless (require 'evil nil t))
  (dolist (reenter '(nil t))
    (voicekey-test-buffer "a b"
      (evil-local-mode 1)
      (if reenter (evil-normal-state) (evil-insert-state))
      (voicekey--pin "first" (voicekey-test-expiry))
      (voicekey--pin "second" (voicekey-test-expiry))
      (when reenter
        (evil-normal-state)
        (evil-insert-state))
      (voicekey--pin "third" (voicekey-test-expiry))
